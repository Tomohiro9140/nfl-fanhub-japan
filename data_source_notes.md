...
## 動画導線とGame Ticketの簡素化（2026-08-16）

390px幅の実データ画面で、個別ハイライト紐付け済み試合のバッジが`リンク済`と表示され、`WATCH HIGHLIGHTS`と同一行で横並びになることを確認した。ネタバレ防止中でもスコアは`RESULT HIDDEN`のままで、動画リンクは引き続き操作可能である。Game Ticketから右上カレンダーアイコン、対戦下のNFL公式日程ソース表記、観戦ボタン下のAPP／BROWSER注記、ネタバレ状態表示を取り除いた。Schedule Deskの試合カードから開始前カウントダウンも削除し、日時とWeek情報のみを残した。
...
既存の認証済みHeartbeatへPFT更新を統合し、ハンドラ統合テストで公式チーム更新、順位・スコア更新、DAZN更新、PFT更新が並行して成功応答へ含まれることを確認した。管理画面の`Run Now`実行後も、この環境のCLIログには新規実行記録が反映されなかったため、実行ログによる追加確認はできなかった。代替として、PFT同期関数を実データで手動実行しBen Brownの外部インサイト保存を確認し、同じ外部IDのユニーク制約により重複保存を防止することを確認した。一時検証Heartbeatは削除済みで、通常の6時間公式更新だけを残している。

## STATUS RADARのPFT文脈判定・ロスター分類修正（2026-08-16）

PFT記事のページ全体を解析していた旧実装では、関連記事・ナビゲーションにある別選手の離脱表現を同じ記事の複数選手へ誤適用する余地があった。記事本文の`article`領域だけを文単位で解析し、選手名と出場可否表現が同じ文にある場合だけ保存する方式へ変更した。Patriotsの実データ画面では、Ben Brownだけが`OUT · MULTI-WEEK`として表示され、Christian Gonzalezへの誤った多週離脱表示は除去された。Buffaloの実データ画面では、ALL／ACTIVE／Reserve種別のタグをロスター一覧フィルターとして表示し、前年の公式負傷記事は45日間の鮮度条件によりSTATUS RADARから除外されることを確認した。

全チームの公式負傷キャッシュを整理し、45日を超える記事を削除した。残存する公式負傷項目は63件で、最も古い公開日時は2026年8月4日となっている。LATEST RESULTS、LATEST NEWS、STATUS RADAR、LEAGUE DESKの再構成に合わせて、モバイルメニューのリンク先と見出し番号を`01`から`04`までに更新した。
...
## ポップアップの言語切替と要約先行生成（2026-08-16）

LATEST NEWSの詳細ポップアップに、日本語要約と英語原文抜粋の切替を追加した。英語本文は公式サイトから一時取得して15分間だけメモリに保持し、最大1,800文字を抜粋表示する。Buffaloの公式記事で英語抜粋1,800文字が取得できることを確認した。ニュース3件は画面表示の700ミリ秒後から低優先度で順番に日本語要約を生成・保存するため、通常の閲覧時はクリック前に要約がキャッシュされる。同一記事で事前生成とクリック時生成が重複しないよう制御し、本文・英語原文はデータベースへ保存しない。

英語原文抜粋はモバイルの詳細ポップアップでは長く、操作を妨げるため、英語表示を公式RSSの短い英語サマリーへ置き換えた。切替ラベルは`ENGLISH SUMMARY`で、詳細は既存の公式サイトリンクから確認できる。これにより英語切替では追加の本文取得を待たず、記事一覧に既に保存されている短い公式概要を即時表示する。

日本語要約もモバイルで過度に長くならないよう、生成目標を160〜300文字へ変更し、保存時の上限を420文字に設定した。既にキャッシュ済みの長い要約も、画面では340文字を上限に文末優先で短く表示する。要約の表示が省略された場合は、公式サイトで詳細を確認する案内を併記する。クライアント側の文末優先圧縮は短文保持と長文省略の回帰テストで検証した。
