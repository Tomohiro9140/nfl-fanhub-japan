...
## 元ATLAS詳細仕様の再確認（2026-08-19）

元ATLASは所属チーム履歴を**新しい年度から古い年度へ**並べ、同じチーム構成の連続年度を一つの期間として畳み込む。`hallOfFameYear`がある選手は、チーム色ではなく金色グラデーションのヒーローカードに`HALL OF FAME`と`CLASS OF <year>`を表示する。古い選手のチーム履歴は、年度別ロスターに加え、保持済みの歴史ロスター索引を優先して補完する。

成績表はポジション別の全指標列を横スクロール表で表示し、1998年以前に該当する、または公開レギュラーシーズン成績がない場合は`1998年以前の成績は表示されません。`と明示する。契約タブでは現役ロスター外の選手に`契約情報は現在ロスターに登録されている選手を対象に表示します`を表示する。
...
2026-08-19の補完実機検証では、ネイティブATLASでTroy Aikman（`00-0000104`）を検索・選択し、金色の`HALL OF FAME`ヒーローカード、`CLASS OF 2006`、Dallas Cowboysの`1989〜2000`経歴を確認した。歴史ロスター索引を現行プロジェクトの永続ストレージへ移設したため、引退・殿堂入り選手についても元ATLASと同じ経歴取得経路を使用する。
