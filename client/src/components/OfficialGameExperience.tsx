import { ArrowUpRight, BarChart3, CircleAlert, Clock3, FileText, UsersRound } from "lucide-react";
...
export function OfficialGameTicket({ favorite, snapshot, loading, spoilerMode = false, hasWatchedTicket = false, canRestoreLastGame = false, onMarkWatched, onRestoreLastGame, onOpenGameStats }: { favorite: FavoriteTeam; snapshot?: TeamSnapshot; loading: boolean; spoilerMode?: boolean; hasWatchedTicket?: boolean; canRestoreLastGame?: boolean; onMarkWatched?: (gameSourceUrl: string) => void; onRestoreLastGame?: () => void; onOpenGameStats?: (gameUrl: string) => void }) {
  ...
  const favoriteScore = game ? (game.homeAway === "home" ? (game.homeScore ?? gameDay?.homeScore) : (game.awayScore ?? gameDay?.awayScore)) : null;
  const opponentScore = game ? (game.homeAway === "home" ? (game.awayScore ?? gameDay?.awayScore) : (game.homeScore ?? gameDay?.homeScore)) : null;
  ...
  return <section data-layout-scope="hero" data-game-ticket-state={gameStatus.label} className="ticket-cut ticket-paper relative overflow-hidden rounded-[18px] bg-[#0a1931] text-[#fffaf0] shadow-[0_24px_50px_rgba(10,25,49,0.2)]">
    <div className="absolute inset-0 bg-[radial-gradient(circle_at_92%_5%,rgba(56,189,248,.19),transparent_30%),linear-gradient(115deg,#0a1931,#112a4b)]" />
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="whitespace-nowrap font-mono text-[10px] font-bold tracking-[.18em] text-[#ffc1a7]">
            GAME TICKET / <span className="inline whitespace-nowrap">{ticketLabel}</span>
          </p>
          <p className="mt-2 font-display text-2xl font-extrabold tracking-[.08em] text-white">
            {isFinalTicket ? "LAST GAME" : gameStatus.label === "LIVE" ? "LIVE GAME" : "NEXT GAME"}
          </p>
        </div>
        <div className="flex shrink-0 flex-col items-start gap-1.5">
          {canOpenGameStats ? spoilerMode ? <span aria-hidden="true" className="inline-flex h-3" /> : <button type="button" onClick={() => { if (game && onOpenGameStats) onOpenGameStats(game.sourceUrl); }} className="inline-flex items-center gap-1 font-mono text-[8px] font-black tracking-[.06em] text-[#ffc1a7] underline underline-offset-2"><BarChart3 className="h-3 w-3" /> GAME STATS</button> : null}
          {isFinalTicket ? game?.nflHighlightUrl ? <a href={game.nflHighlightUrl} target="_blank" rel="noreferrer" className="font-mono text-[8px] font-bold text-[#ffc1a7] underline underline-offset-2">WATCH HIGHLIGHTS <ArrowUpRight className="inline h-3 w-3" /></a> : <span className="font-mono text-[8px] font-bold tracking-[.06em] text-[#a5b3c9]">HIGHLIGHTS · 準備中</span> : null}
          {isFinalTicket && game && onMarkWatched ? <button type="button" onClick={() => onMarkWatched(game.sourceUrl)} className="font-mono text-[8px] font-bold text-[#d9e3f3] underline underline-offset-2">ON TO THE NEXT GAME</button> : null}
        ...
        <div className={`mt-2 grid grid-cols-[1fr_auto_1fr] items-center gap-3 ${finalTeamRowClass}`}>
          ...
          <div className={`border-x border-dashed border-white/25 px-3 text-center ${isFinalTicket ? "flex h-[72px] flex-col justify-center" : ""}`}>
            {gameStatus.label === "FINAL" ? <>
              ...
              <p className="mt-1 font-display text-lg font-extrabold leading-none">{fmtFinalGameDate(game.kickoffAt ?? gameDay?.kickoffAt, game.gameDate ?? gameDay?.gameDate)}</p>
...
function gameStateCopy(game?: TeamSnapshot["gameDayStatus"], hideScores = false) {
  ...
  if (state.includes("LIVE") || state.includes("INGAME") || state.includes("IN_PROGRESS") || state.includes("HALFTIME")) {
    return { label: "LIVE", detail: hideScores ? "ネタバレ防止中 · スコア非表示" : "公式スコアを更新中", score: hideScores ? null : score };
  }
