...
export function OfficialGameTicket({ favorite, snapshot, loading, spoilerMode }: { favorite: FavoriteTeam; snapshot?: TeamSnapshot; loading: boolean; spoilerMode: boolean }) {
  ...
  const location = game?.homeAway === "away" ? "@" : "VS";
  return <section data-layout-scope="hero" className="ticket-cut ticket-paper relative overflow-hidden rounded-[18px] bg-[#0a1931] text-[#fffaf0] shadow-[0_24px_50px_rgba(10,25,49,0.2)]">
    ...
    <div className="relative p-4 sm:p-5">
      <div className="flex items-start justify-between gap-4"><div><p className="font-mono text-[10px] font-bold tracking-[.18em] text-[#ffc1a7]">GAME TICKET / {game ? `${game.seasonPhase.toUpperCase()} ${game.weekLabel ?? ""}` : "OFFICIAL SCHEDULE"}</p><p className="mt-2 font-display text-2xl font-extrabold tracking-[.08em] text-white">{game && opponent ? `${favorite.code} ${location} ${opponent.code}` : `${favorite.code} / NEXT GAME`}</p></div><CalendarDays className="mt-1 h-5 w-5 text-[#ffc1a7]" /></div>
      {loading ? <p className="mt-5 font-mono text-[11px] text-[#d9e3f3]">LOADING OFFICIAL SCHEDULE…</p> : game && opponent ? <><div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-3"><div><p className="font-display text-lg font-bold">{favorite.code}</p><p className="text-[11px] text-[#a5b3c9]">{favorite.name}</p></div><div className="border-x border-dashed border-white/25 px-3 text-center"><p className="font-mono text-[9px] font-semibold tracking-[.14em] text-[#a5b3c9]">JST</p><p className="mt-1 font-display text-lg font-extrabold leading-none">{fmtDate(game.kickoffAt)}</p><p className="mt-1 text-[10px] text-[#ffc1a7]">{game.venue ?? "VENUE TBA"}</p></div><div className="text-right"><p className="font-display text-lg font-bold">{opponent.code}</p><p className="text-[11px] text-[#a5b3c9]">{opponent.name}</p></div></div><p className="mt-3 font-mono text-[9px] tracking-[.08em] text-[#d9e3f3]">{game.broadcast ?? "BROADCAST TBA"} · OFFICIAL TEAM SCHEDULE</p></> : <div className="mt-4"><EmptyOfficial label="OFFICIAL SCHEDULE PENDING" copy="チーム公式の次戦情報を取得後に表示します。" /></div>}
      <div className="ticket-rule mt-4 pt-3"><a href={daznGamePassUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-10 w-full items-center justify-center gap-2 bg-[#e85d2a] px-3 font-sans text-[13px] font-bold text-white transition hover:bg-[#cf4f20] active:scale-[.97]"><Tv className="h-4 w-4" /> 観戦する <ArrowUpRight className="h-4 w-4" /></a></div>
