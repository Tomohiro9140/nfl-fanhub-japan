...
export function OfficialGameTicket({ favorite, snapshot, loading }: { favorite: FavoriteTeam; snapshot?: TeamSnapshot; loading: boolean }) {
  ...
  return <section data-layout-scope="hero" className="ticket-cut ticket-paper relative overflow-hidden rounded-[18px] bg-[#0a1931] text-[#fffaf0] shadow-[0_24px_50px_rgba(10,25,49,0.2)]">
    ...
    <div className="relative p-4 sm:p-5">
      <div className="flex items-start justify-between gap-3"><div><p className="font-mono text-[10px] font-bold tracking-[.18em] text-[#ffc1a7]">GAME TICKET / {game ? (game.weekLabel ?? game.seasonPhase.toUpperCase()) : "OFFICIAL SCHEDULE"}</p><p className="mt-2 font-display text-2xl font-extrabold tracking-[.08em] text-white">NEXT GAME</p></div><a href={watchUrl} target={watchTarget} rel="noreferrer" className="inline-flex shrink-0 items-center gap-1 border border-[#ffc1a7]/70 bg-white/10 px-2 py-1.5 font-mono text-[9px] font-bold tracking-[.08em] text-white transition hover:bg-white/20" aria-label="DAZN NFL Game Passホームを開いて観戦する"><Tv className="h-3.5 w-3.5 text-[#ffc1a7]" /> DAZN <ArrowUpRight className="h-3.5 w-3.5" /></a></div>
      {loading ? <p className="mt-5 font-mono text-[11px] text-[#d9e3f3]">LOADING OFFICIAL SCHEDULE…</p> : game && opponent ? <><div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-3"><div><p className="font-display text-base font-bold leading-tight">{favoriteLabel}</p></div><div className="border-x border-dashed border-white/25 px-3 text-center"><p className="font-mono text-[9px] font-semibold tracking-[.14em] text-[#a5b3c9]">JST</p><p className="mt-1 font-display text-lg font-extrabold leading-none">{fmtDate(game.kickoffAt)}</p><GameCountdown kickoffAt={game.kickoffAt} result={{ gameState: game.gameState, awayScore: game.awayScore, homeScore: game.homeScore }} className="mt-1 text-[9px]" />{confirmedVenue(game.venue) ? <p className="mt-1 text-[10px] text-[#ffc1a7]">{confirmedVenue(game.venue)}</p> : null}</div><div className="text-right"><p className="font-display text-base font-bold leading-tight">{opponentLabel}</p></div></div>{game.broadcast ? <p className="mt-3 font-mono text-[9px] tracking-[.08em] text-[#d9e3f3]">{game.broadcast}</p> : null}</> : <div className="mt-4"><EmptyOfficial label="OFFICIAL SCHEDULE PENDING" copy="NFL公式リーグ日程とチーム公式Scheduleを確認後に表示します。" /></div>}
...
function gameStateCopy(game?: TeamSnapshot["gameDayStatus"]) {
  ...
  if (state === "FINAL" || state === "COMPLETED") return { label: "FINAL", detail: "公式スコアを確認済み", score };
...
export function OfficialGameDayStatus({ favorite, snapshot, loading }: { favorite: FavoriteTeam; snapshot?: TeamSnapshot; loading: boolean }) {
  const game = snapshot?.gameDayStatus;
  const opponent = getTeamByCode(game?.opponentCode ?? null);
  const status = gameStateCopy(game);
  const favoriteLabel = game?.homeAway === "home" ? `@ ${favorite.name}` : favorite.name;
  const opponentLabel = game?.homeAway === "away" && opponent ? `@ ${opponent.name}` : opponent?.name;
  return <section data-layout-scope="game-day-status" className="border border-[#d9d5cc] bg-[#fffdf8] p-3 shadow-[0_10px_30px_rgba(34,42,53,.04)]"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-[9px] font-bold tracking-[.15em] text-[#64748b]">OFFICIAL NFL / GAME DAY</p><h2 className="mt-1 font-display text-lg font-extrabold tracking-[.06em]">GAME-DAY STATUS</h2></div><span className={`rounded px-2 py-1 font-mono text-[9px] font-bold ${status.label === "LIVE" ? "bg-[#e85d2a] text-white" : status.label === "FINAL" ? "bg-[#10213a] text-white" : "bg-[#e8f0fb] text-[#365077]"}`}>{status.label}</span></div>{loading ? <p className="mt-3 font-mono text-[10px] text-[#687587]">LOADING OFFICIAL GAME STATUS…</p> : game && opponent ? <div className="mt-3 grid grid-cols-[1fr_auto_1fr] items-center gap-2 border-y border-[#eeeae1] py-2.5"><p className="text-[12px] font-bold leading-tight">{favoriteLabel}</p><div className="px-2 text-center"><p className="font-display text-base font-extrabold">{status.score ?? fmtDate(game.kickoffAt)}</p><p className="mt-0.5 font-mono text-[8px] tracking-[.08em] text-[#687587]">{game.weekLabel ?? "OFFICIAL GAME"}</p></div><p className="text-right text-[12px] font-bold leading-tight">{opponentLabel}</p></div> : <p className="mt-3 text-[11px] text-[#687587]">次の公式ゲーム情報を確認中です。</p>}<div className="mt-2 flex items-center justify-between gap-2"><p className="text-[10px] text-[#687587]">{status.detail}</p><a href="https://www.nfl.com/inactives/" target="_blank" rel="noreferrer" className="shrink-0 font-mono text-[9px] font-bold text-[#365077] underline decoration-[#e85d2a] decoration-2 underline-offset-3">OFFICIAL INACTIVES <ArrowUpRight className="inline h-3 w-3" /></a></div>{game?.sourceUrl ? <a href={game.sourceUrl} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 font-mono text-[9px] text-[#687587] underline underline-offset-2">OFFICIAL GAME CENTER <ArrowUpRight className="h-3 w-3" /></a> : null}</section>;
}
