...
export function OfficialGameTicket({ favorite, snapshot, loading, spoilerMode = false, onMarkWatched }: { favorite: FavoriteTeam; snapshot?: TeamSnapshot; loading: boolean; spoilerMode?: boolean; onMarkWatched?: (gameSourceUrl: string) => void }) {
  ...
  const replayUrl = game?.daznUrl ?? daznGamePassUrl;
  return <section data-layout-scope="hero" className="ticket-cut ticket-paper relative overflow-hidden rounded-[18px] bg-[#0a1931] text-[#fffaf0] shadow-[0_24px_50px_rgba(10,25,49,0.2)]">
    ...
    <div className="relative p-4 sm:p-5">
      <div className="flex items-start justify-between gap-3"><div><p className="font-mono text-[10px] font-bold tracking-[.18em] text-[#ffc1a7]">GAME TICKET / {game ? (game.weekLabel ?? game.seasonPhase.toUpperCase()) : "OFFICIAL SCHEDULE"}</p><p className="mt-2 font-display text-2xl font-extrabold tracking-[.08em] text-white">{gameStatus.label === "FINAL" ? "LAST GAME" : "NEXT GAME"}</p></div><a href={watchUrl} target={watchTarget} rel="noreferrer" className="inline-flex shrink-0 items-center gap-1 border border-[#ffc1a7]/70 bg-white/10 px-2 py-1.5 font-mono text-[9px] font-bold tracking-[.08em] text-white transition hover:bg-white/20" aria-label="DAZN NFL Game Passホームを開いて観戦する"><Tv className="h-3.5 w-3.5 text-[#ffc1a7]" /> DAZN <ArrowUpRight className="h-3.5 w-3.5" /></a></div>
      {loading ? <p className="mt-5 font-mono text-[11px] text-[#d9e3f3]">LOADING OFFICIAL SCHEDULE…</p> : game && opponent ? <><div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-3"><div><p className="font-display text-base font-bold leading-tight">{favoriteLabel}</p></div><div className="border-x border-dashed border-white/25 px-3 text-center">{isRevealedFinal ? <><p className="-mt-1 font-mono text-[9px] font-semibold tracking-[.14em] text-[#a5b3c9]">{fmtGameDate(game.kickoffAt)}</p><p className="mt-1 font-display text-3xl font-black leading-none tracking-[.03em] text-white">{gameStatus.score}</p><p className="mt-1 font-mono text-[8px] font-bold tracking-[.14em] text-[#ffc1a7]">FINAL SCORE</p></> : <><p className="font-mono text-[9px] font-semibold tracking-[.14em] text-[#a5b3c9]">JST</p><p className="mt-1 font-display text-lg font-extrabold leading-none">{fmtDate(game.kickoffAt)}</p><GameCountdown kickoffAt={game.kickoffAt} result={{ gameState: game.gameState, awayScore: game.awayScore, homeScore: game.homeScore }} hideFinalScore={spoilerMode} className="mt-1 text-[9px]" />{confirmedVenue(game.venue) ? <p className="mt-1 text-[10px] text-[#ffc1a7]">{confirmedVenue(game.venue)}</p> : null}</>}</div><div className="text-right"><p className="font-display text-base font-bold leading-tight">{opponentLabel}</p></div></div>{game.broadcast ? <p className="mt-3 font-mono text-[9px] tracking-[.08em] text-[#d9e3f3]">{game.broadcast}</p> : null}<div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-white/15 pt-2 font-mono text-[9px]"><span className="font-bold tracking-[.1em] text-[#a5b3c9]">GAME STATUS</span><span className={`rounded px-1.5 py-0.5 font-bold ${gameStatus.label === "LIVE" ? "bg-[#e85d2a] text-white" : gameStatus.label === "FINAL" ? "bg-white text-[#10213a]" : "bg-[#315272] text-white"}`}>{gameStatus.label}</span><span className="text-[#d9e3f3]">{isRevealedFinal ? "OFFICIAL SCORE CONFIRMED" : gameStatus.score ? `OFFICIAL SCORE ${gameStatus.score}` : gameStatus.detail}</span><a href="https://www.nfl.com/inactives/" target="_blank" rel="noreferrer" className="font-bold text-[#ffc1a7] underline underline-offset-2">INACTIVES <ArrowUpRight className="inline h-3 w-3" /></a>{gameDay?.sourceUrl ? <a href={gameDay.sourceUrl} target="_blank" rel="noreferrer" className="font-bold text-[#ffc1a7] underline underline-offset-2">GAME CENTER <ArrowUpRight className="inline h-3 w-3" /></a> : null}</div>{gameStatus.label === "FINAL" ? <div className="mt-2 flex flex-wrap items-center gap-2 font-mono text-[9px] font-bold"><a href={replayUrl} target={watchTarget} rel="noreferrer" className="border border-[#ffc1a7]/70 bg-white/10 px-2 py-1.5 text-[#fffaf0] transition hover:bg-white/20">DAZN REPLAY <ArrowUpRight className="inline h-3 w-3" /></a>{onMarkWatched ? <button type="button" onClick={() => onMarkWatched(game.sourceUrl)} className="border border-white/30 px-2 py-1.5 text-[#d9e3f3] transition hover:bg-white/10">VIEWED · SHOW NEXT</button> : null}</div> : null}</> : <div className="mt-4"><EmptyOfficial label="OFFICIAL SCHEDULE PENDING" copy="NFL公式リーグ日程とチーム公式Scheduleを確認後に表示します。" /></div>}
