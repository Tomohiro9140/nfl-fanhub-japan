import { BarChart3, CalendarDays, ChevronRight, CircleAlert, Eye, ListOrdered, Trophy } from "lucide-react";
...
function calendarDate(value: Date) {
  ...
  const timeStr = new Intl.DateTimeFormat("ja-JP", { hour: "2-digit", minute: "2-digit", timeZone: "Asia/Tokyo" }).format(d);
    ...
    <section id="results" data-layout-scope="latest-results" className="scroll-mt-24">
      ...
      <div className="rounded-[2px_14px_2px_14px] border border-[#ded8cc] bg-[#fffdf8] px-3 py-2 shadow-[0_10px_30px_rgba(34,42,53,.05)]">
        ...
        {loading ? (
          ...
          <p className="mt-1.5 border border-dashed border-[#d7d1c4] bg-[#fffdf8] p-2 text-[11px] leading-5 text-[#687587]">NFL公式スコアの初回同期を待っています。</p>
        )}
...
export function OfficialLeagueDashboard({ favorite, dashboard, loading, calendarLoading = loading, spoilerMode = false }: { favorite: FavoriteTeam; dashboard?: LeagueDashboard; loading: boolean; calendarLoading?: boolean; spoilerMode?: boolean }) {
  const [calendarView, setCalendarView] = useState<"team" | "week">("team");
  const [revealedStandings, setRevealedStandings] = useState(false);

  // ネタバレ防止の切り替えや推しチーム変更時に自動で再マスク
  useEffect(() => {
    setRevealedStandings(false);
  }, [spoilerMode, favorite.code]);

  const isStandingsMasked = spoilerMode && !revealedStandings;
  ...
  const calendarCard = (game: LeagueCalendarGame) => {
    ...
    return <a key={game.id} href={href} target={target} rel="noreferrer" style={brandStyle} className={`min-w-0 border-l-2 px-2 py-2 ${venueStyle}`}><p className="truncate font-mono text-[11px] font-bold">{abbreviatedMatchup(game)}</p><p className={`mt-0.5 truncate text-[9px] ${usesFavoriteBrand ? "opacity-80" : "text-[#5b6472]"}`}>{game.liveScoreboardFallback ? "LIVE · OFFICIAL SCOREBOARD" : calendarDate(game.kickoffAt)}</p><p className={`mt-1 truncate font-mono text-[9px] font-bold tracking-[.04em] ${usesFavoriteBrand ? "opacity-90" : "text-[#4b5563]"}`}>{seasonWeekLabel(game)}{game.broadcast ? ` · ${game.broadcast}` : ""}</p></a>;
  };

  return (
    <section id="league" data-layout-scope="league-dashboard" className="scroll-mt-24 space-y-3">
      <div className="flex items-center gap-2 font-mono text-[10px] font-semibold tracking-[0.2em] text-[#64748b]">
        <span className="text-[#10213a]">04</span><span>LEAGUE DESK</span><span className="h-px flex-1 bg-[#d9d5cc]" />
      </div>
      <article className="clip-note border border-[#ded8cc] bg-white p-3 shadow-[0_10px_30px_rgba(34,42,53,.05)]">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="font-mono text-[9px] font-bold tracking-[.14em] text-[#64748b]">NFL OFFICIAL</p>
            <h2 className="mt-1 font-display text-xl font-extrabold tracking-wide">DIVISION STANDINGS</h2>
            <p className="mt-1 text-[10px] text-[#687587]">
              {favorite.conference} {favorite.division.toUpperCase()} · UPDATED {sourceTime(dashboard?.lastUpdatedAt)} JST
            </p>
          </div>
          <ListOrdered className="h-5 w-5 text-[#e85d2a]" />
        </div>

        {/* 順位表 + すりガラスマスク */}
        <div className="relative mt-3">
          <div className={isStandingsMasked ? "filter blur-[5px] select-none pointer-events-none transition-all duration-200" : "transition-all duration-200"}>
            {loading ? (
              <p className="py-5 text-center font-mono text-[10px] text-[#64748b]">LOADING OFFICIAL STANDINGS…</p>
            ) : divisionStandings.some((item) => item.row) ? (
              <div className="divide-y divide-[#eeeae1]">
                {divisionStandings.map(({ team, row }, index) => (
                  <div key={team.code} className={`grid grid-cols-[18px_1fr_auto] items-center gap-2 py-2.5 ${team.code === favorite.code ? "bg-[#fff8ed]" : ""}`}>
                    <span className="font-mono text-[10px] text-[#64748b]">{index + 1}</span>
                    <p className="truncate text-[12px] font-bold">{team.name}</p>
                    <p className="font-mono text-[11px] font-bold">{row ? `${row.wins}-${row.losses}${row.ties ? `-${row.ties}` : ""}` : "—"}</p>
                  </div>
                ))}
              </div>
            ) : (
              <div className="border border-dashed border-[#d7d1c4] bg-[#fffdf8] p-3 text-[11px] leading-5 text-[#687587]">
                <CircleAlert className="mr-1 inline h-3.5 w-3.5 text-[#e85d2a]" />
                NFL公式順位表の初回同期を待っています。
              </div>
            )}
            <a href="https://www.nfl.com/standings/league/2026/REG" target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-1 text-[10px] font-bold underline decoration-[#e85d2a] decoration-2 underline-offset-3">
              OFFICIAL STANDINGS <ChevronRight className="h-3 w-3" />
            </a>
          </div>

          {/* ネタバレ防止マスク（オーバーレイ） */}
          {isStandingsMasked && (
            <div className="absolute inset-0 z-10 flex flex-col items-center justify-center rounded bg-white/75 p-4 text-center backdrop-blur-[3px]">
              <button
                type="button"
                onClick={() => setRevealedStandings(true)}
                className="inline-flex items-center gap-2 rounded border border-[#b8dca8] bg-[#f0f8eb] px-3.5 py-2 text-xs font-bold text-[#2d5220] shadow-sm transition hover:bg-[#e4f3dc] active:scale-95"
              >
                <Eye className="h-4 w-4 text-[#3d6b2c]" />
                <span>タップして順位を表示</span>
              </button>
              <p className="mt-2 font-mono text-[9px] font-semibold tracking-wider text-[#64748b]">
      ...
      <article className="border border-[#ded8cc] bg-white p-3 shadow-[0_10px_30px_rgba(34,42,53,.05)]">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="font-mono text-[9px] font-bold tracking-[.14em] text-[#64748b]">NFL OFFICIAL SCHEDULE</p>
            <h2 className="mt-1 font-display text-xl font-extrabold tracking-wide">SCHEDULE DESK</h2>
          </div>
          <CalendarDays className="h-5 w-5 text-[#e85d2a]" />
        </div>
        {calendarLoading ? (
          <p className="py-5 text-center font-mono text-[10px] text-[#64748b]">LOADING OFFICIAL CALENDAR…</p>
        ) : (
          <>
            <div className="mt-3 grid grid-cols-2 gap-1.5" role="tablist" aria-label="Schedule view">
              <button role="tab" aria-selected={calendarView === "team"} onClick={() => setCalendarView("team")} className={`border px-2 py-2 font-mono text-[9px] font-bold leading-3 ${calendarView === "team" ? "border-[#10213a] bg-[#10213a] text-white" : "border-[#d7d1c4] bg-[#fffdf8] text-[#526173]"}`}>
                MY TEAM<br />FULL SCHEDULE
              </button>
              <button role="tab" aria-selected={calendarView === "week"} onClick={() => setCalendarView("week")} className={`border px-2 py-2 font-mono text-[9px] font-bold leading-3 ${calendarView === "week" ? "border-[#10213a] bg-[#10213a] text-white" : "border-[#d7d1c4] bg-[#fffdf8] text-[#526173]"}`}>
                ALL GAMES<br />NEXT 7 DAYS
              </button>
            </div>
            <p className="mt-2 font-mono text-[9px] font-bold tracking-[.1em] text-[#64748b]">
              {calendarView === "team" ? `${favorite.code} · ALL SCHEDULED GAMES` : "ALL TEAMS · ROLLING 7-DAY WINDOW"}
            </p>
                ...
                <div className="mt-2 space-y-3">
                  {groupedWeekGames.map((group) => (
                    <section key={group.label}>
                      <p className="mb-1 font-mono text-[9px] font-bold tracking-[.12em] text-[#526173]">GAME DAY / {group.label}</p>
                      <div className="grid grid-cols-2 gap-1.5 sm:gap-2">{group.games.map(calendarCard)}</div>
                    </section>
                  ))}
                </div>
              ) : (
                <div className="mt-2 grid grid-cols-2 gap-1.5 sm:gap-2">{calendarGames.map(calendarCard)}</div>
              )
            ) : (
              <p className="mt-3 border border-dashed border-[#d7d1c4] bg-[#fffdf8] p-3 text-[11px] leading-5 text-[#687587]">
                {calendarView === "team" ? `${favorite.code}の公式日程は次回同期で表示します。` : "今後7日間にNFL公式日程の試合はありません。"}
              </p>
            )}
          </>
        )}
        <a href="https://www.nfl.com/schedules" target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-1 text-[10px] font-bold underline decoration-[#e85d2a] decoration-2 underline-offset-3">
          OFFICIAL SCHEDULE <ChevronRight className="h-3 w-3" />
        </a>
      </article>
    </section>
  );
