...
import { OfficialGameTicket, OfficialStatusRadar, type TeamSnapshot } from "@/components/OfficialGameExperience";
...
export default function Home() {
  ...
  return <div className="min-h-screen overflow-x-clip bg-[#f5f2ea] text-[#10213a] selection:bg-[#e85d2a] selection:text-white">
    ...
    <main className="relative z-10 mx-auto w-full min-w-0 max-w-6xl overflow-x-clip px-4 pb-24 pt-3 sm:px-6 sm:pt-5"><div className="mb-3 flex items-center justify-between gap-4"><p className="font-mono text-[10px] font-bold tracking-[.16em] text-[#64748b]">JST · OFFICIAL DATA</p><p className="flex items-center gap-1.5 font-mono text-[10px] font-bold text-[#526173]"><span className={`h-1.5 w-1.5 rounded-full ${snapshotQuery.isError || leagueQuery.isError ? "bg-[#e85d2a]" : "bg-[#69a84c]"}`} /> {snapshotQuery.isError || leagueQuery.isError ? "CACHE UNAVAILABLE" : "OFFICIAL CACHE"}</p></div>{snapshotQuery.isError && <div className="mb-3 flex items-center gap-2 border border-[#f1c7b5] bg-[#fff4ef] px-3 py-2 text-[11px] text-[#a34220]"><CircleAlert className="h-4 w-4" />公式データを一時的に取得できません。保存済みの情報を再試行します。</div>}<div className="min-w-0 space-y-3"><OfficialGameTicket favorite={favorite} snapshot={displaySnapshot} loading={snapshotQuery.isLoading} /><SpoilerSwitch spoilerMode={spoilerMode} onToggle={toggleSpoiler} /><OfficialLatestResults favorite={favorite} dashboard={leagueDashboard} loading={leagueQuery.isLoading} spoilerMode={spoilerMode} /><div data-layout-scope="official-feed" className="min-w-0"><OfficialTeamFeed favorite={favorite} /></div><OfficialStatusRadar favorite={favorite} snapshot={displaySnapshot} loading={snapshotQuery.isLoading} /></div><div className="mt-3"><OfficialLeagueDashboard favorite={favorite} dashboard={leagueDashboard} loading={leagueQuery.isLoading} /></div><section className="mt-4 border-t border-[#d8d1c3] pt-3"><p className="text-[11px] text-[#687587]">NFL公式リーグ日程・スコア・順位表を優先し、チーム公式Schedule、Roster、News、NFL公式Injury Reportで補完します。更新時刻は各カードに表示されます。</p></section></main>
  </div>;
}
