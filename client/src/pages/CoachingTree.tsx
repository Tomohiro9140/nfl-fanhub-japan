...
const teamOptions = Array.from(new Set(centralAtlasEdges.filter((edge) => hierarchyRelationTypes.has(edge.relationType)).flatMap((edge) => edge.team.split(" / ").map((team) => team.trim())))).sort((a, b) => a.localeCompare(b));
...
export default function Home() {
  ...
  const relationTeams = useMemo(() => Array.from(new Set(activeTreeEdges.map((edge) => edge.team))).sort((a, b) => a.localeCompare(b)), [activeTreeEdges]);
  ...
  function staffRecordForMapRelation(relation: CentralAtlasEdge) {
    ...
    return candidates.find((candidate) => (fromName && recordHasHeadCoach(candidate, fromName)) || (toName && recordHasHeadCoach(candidate, toName))) ?? candidates[0];
  }
