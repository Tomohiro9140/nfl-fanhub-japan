...
import { ArrowLeftRight, CalendarRange, ShieldAlert, Trophy } from "lucide-react";
...
const metricGroups: { title: string; metrics: { key: MetricKey; label: string; format: "number" | "decimal" | "percent" | "epa" }[] }[] = [
  { title: "OFFENSE", metrics: [{ key: "pointsPerGame", label: "得点 / G", format: "decimal" }, { key: "yardsPerGame", label: "獲得ヤード / G", format: "decimal" }, { key: "epaPerPlay", label: "EPA / Play", format: "epa" }, { key: "passYardsPerGame", label: "パスヤード / G", format: "decimal" }, { key: "rushYardsPerGame", label: "ランヤード / G", format: "decimal" }, { key: "passerRating", label: "Passer Rating", format: "decimal" }, { key: "thirdDownPct", label: "3rd Down", format: "percent" }, { key: "redZoneTdPct", label: "RZ TD", format: "percent" }, { key: "sacksAllowed", label: "被サック数", format: "number" }] },
  { title: "DEFENSE", metrics: [{ key: "pointsAllowedPerGame", label: "失点 / G", format: "decimal" }, { key: "yardsAllowedPerGame", label: "喪失ヤード / G", format: "decimal" }, { key: "opponentEpaPerPlay", label: "Opponent EPA / Play", format: "epa" }, { key: "passYardsAllowedPerGame", label: "パス喪失ヤード / G", format: "decimal" }, { key: "rushYardsAllowedPerGame", label: "ラン喪失ヤード / G", format: "decimal" }, { key: "opponentThirdDownPct", label: "Opponent 3rd Down", format: "percent" }, { key: "opponentRedZoneTdPct", label: "Opponent RZ TD", format: "percent" }, { key: "sacksDefense", label: "Sacks", format: "number" }, { key: "interceptionsDefense", label: "INT", format: "number" }, { key: "turnovers", label: "Takeaways", format: "number" }] },
  { title: "SPECIAL TEAMS", metrics: [{ key: "fieldGoalPct", label: "FG%", format: "percent" }, { key: "extraPointPct", label: "XP%", format: "percent" }, { key: "puntInside20Pct", label: "Punt In-20%", format: "percent" }] },
  { title: "DISCIPLINE", metrics: [{ key: "penalties", label: "Penalties", format: "number" }] },
];
const lowerIsBetter = new Set<MetricKey>(["sacksAllowed", "pointsAllowedPerGame", "yardsAllowedPerGame", "opponentEpaPerPlay", "passYardsAllowedPerGame", "rushYardsAllowedPerGame", "opponentThirdDownPct", "opponentRedZoneTdPct", "penalties"]);
...
function TeamMark({ code, size = "md" }: { code: string; size?: "sm" | "md" | "lg" }) {
  ...
  return brand ? <img src={brand.logo} alt={`${code} logo`} className={`${dimension} shrink-0 object-contain drop-shadow-sm`} style={{ mixBlendMode: "multiply" }} /> : <span className={`${dimension} inline-flex items-center justify-center rounded-full bg-slate-200 text-[9px] font-bold text-slate-600`}>{code}</span>;
}
const MemoTeamMark = memo(TeamMark);

function getStoredTeam(side: "left" | "right", fallback: string) {
  if (typeof window === "undefined") return fallback;
  try { const saved = JSON.parse(window.localStorage.getItem(storageKey) ?? "{}"); return typeof saved[`${side}Team`] === "string" && saved[`${side}Team`] in fieldlineTeamBrand ? saved[`${side}Team`] : fallback; } catch { return fallback; }
}
...
function formatRecord(record: { wins: number; losses: number; ties: number }) { return `${record.wins}勝 ${record.losses}敗${record.ties ? ` ${record.ties}分` : ""}`; }
function formatLastUpdatedAt(value: Date | null | undefined) { return value ? new Intl.DateTimeFormat("ja-JP", { dateStyle: "short", timeStyle: "short", timeZone: "Asia/Tokyo" }).format(new Date(value)) : null; }
function isSuperior(value: number | null | undefined, opponent: number | null | undefined, metric: MetricKey) { return value !== null && value !== undefined && opponent !== null && opponent !== undefined && value !== opponent && (lowerIsBetter.has(metric) ? value < opponent : value > opponent); }
function useDebouncedSelection(selection: Selection, delay = 180) { const [debounced, setDebounced] = useState(selection); useEffect(() => { const timer = window.setTimeout(() => setDebounced(selection), delay); return () => window.clearTimeout(timer); }, [selection, delay]); return debounced; }
...
function Unavailable({ side, message }: { side: string; message?: string }) { return <Card className="border-dashed border-slate-300 bg-slate-50/75"><CardContent className="flex min-h-52 flex-col items-center justify-center px-8 py-10 text-center"><ShieldAlert className="mb-3 h-7 w-7 text-amber-600" /><h3 className="font-semibold text-slate-800">{side}のデータは未取得です</h3><p className="mt-2 max-w-md text-sm leading-6 text-slate-500">{message ?? "この選択条件には集計済みの成績がありません。"}</p></CardContent></Card>; }

function MetricGroup({ group, left, right }: { group: typeof metricGroups[number]; left: any; right: any }) { return <section className="overflow-hidden rounded-[1.4rem] border border-slate-200 bg-white shadow-[0_12px_30px_rgba(15,23,42,.05)]"><div className="flex items-center border-b border-slate-100 bg-slate-50/70 px-5 py-2.5"><p className="text-[11px] font-bold tracking-[.16em] text-slate-500">{group.title}</p></div><div>{group.metrics.map(metric => { const leftValue = left.summary.metrics[metric.key]; const rightValue = right.summary.metrics[metric.key]; const leftSuperior = isSuperior(leftValue, rightValue, metric.key); const rightSuperior = isSuperior(rightValue, leftValue, metric.key); return <div key={metric.key} className="grid min-h-12 grid-cols-[1fr_auto_1fr] items-center gap-3 border-b border-slate-100 px-5 py-2 last:border-0"><div className="text-right"><span className={`inline-block rounded-md text-lg font-semibold tabular-nums ${leftSuperior ? "bg-emerald-50 px-1.5 py-0.5 text-emerald-700 ring-1 ring-emerald-200" : "text-slate-900"}`}>{formatMetric(leftValue, metric.format)}</span><span className="ml-2 text-xs font-medium tabular-nums text-slate-400">#{left.summary.ranks[metric.key] ?? "—"}</span></div><div className="min-w-[7.4rem] text-center"><p className="text-xs font-semibold text-slate-700">{metric.label}</p></div><div><span className={`inline-block rounded-md text-lg font-semibold tabular-nums ${rightSuperior ? "bg-emerald-50 px-1.5 py-0.5 text-emerald-700 ring-1 ring-emerald-200" : "text-slate-900"}`}>{formatMetric(rightValue, metric.format)}</span><span className="ml-2 text-xs font-medium tabular-nums text-slate-400">#{right.summary.ranks[metric.key] ?? "—"}</span></div></div>; })}</div></section>; }

function ComparisonTable({ data }: { data: any }) { const left = data?.left; const right = data?.right; if (!left?.available || !right?.available) return <div className="grid gap-5 lg:grid-cols-2"><Unavailable side="左パネル" message={left?.reason} /><Unavailable side="右パネル" message={right?.reason} /></div>; return <div className="space-y-4"><div className="grid grid-cols-2 gap-1.5 rounded-xl border border-slate-200 bg-white p-1.5"><div className="flex min-w-0 items-center gap-1.5 rounded-lg bg-slate-50 px-2 py-1.5"><MemoTeamMark code={left.summary.team} size="sm" /><p className="min-w-0 truncate text-xs font-semibold text-slate-900">{left.summary.teamName}</p></div><div className="flex min-w-0 items-center justify-end gap-1.5 rounded-lg bg-slate-50 px-2 py-1.5 text-right"><p className="min-w-0 truncate text-xs font-semibold text-slate-900">{right.summary.teamName}</p><MemoTeamMark code={right.summary.team} size="sm" /></div></div><div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-2.5 shadow-[0_6px_18px_rgba(15,23,42,.04)]"><p className="text-center text-base font-bold tabular-nums text-slate-900">{formatRecord(left.summary.record)}</p><p className="text-[10px] font-bold tracking-[.12em] text-slate-400">勝敗</p><p className="text-center text-base font-bold tabular-nums text-slate-900">{formatRecord(right.summary.record)}</p></div>{metricGroups.map(group => <MetricGroup key={group.title} group={group} left={left} right={right} />)}</div>; }

function ComparisonSkeleton() { return <div className="space-y-4" aria-label="比較結果を読み込んでいます" aria-busy="true">{[0, 1, 2, 3].map(item => <section key={item} className="overflow-hidden rounded-[1.4rem] border border-slate-200 bg-white"><div className="h-9 border-b border-slate-100 bg-slate-50/70" /><div className="space-y-3 px-5 py-3">{[0, 1, 2].map(row => <div key={row} className="grid grid-cols-[1fr_auto_1fr] items-center gap-3"><div className="ml-auto h-5 w-20 animate-pulse rounded bg-slate-100" /><div className="h-3 w-24 animate-pulse rounded bg-slate-100" /><div className="h-5 w-20 animate-pulse rounded bg-slate-100" /></div>)}</div></section>)}</div>; }
...
export default function Fieldline() {
  ...
  const leftWeeksQuery = trpc.fieldline.weeks.useQuery(leftWeekInput, weekQueryOptions); const rightWeeksQuery = trpc.fieldline.weeks.useQuery(rightWeekInput, weekQueryOptions);
  ...
  const comparison = trpc.fieldline.compare.useQuery(compareInput, { ...comparisonQueryOptions, enabled: selectionsInSync && !missingWeeks && leftReady && rightReady });
