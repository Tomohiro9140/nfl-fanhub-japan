...
/**
 * 元のRSS取得・パース関数を復元（fetchRss などをファイル内に配置）
 */
...
// 元のパース関数を復元し、SourceKind: local に対応
function parseExternalTeamNewsRss(
  xml: string,
  source: { kind: "pft" | "cbs" | "local"; name: string; url: string },
  favoriteTeamCode: FavoriteTeamCode,
  now: Date
): InsertOfficialFeedItem[] {
  const parser = new XMLParser.XMLParser({ ignoreAttributes: false });
  ...
  return (Array.isArray(items) ? items : [items]).map((item: any) => ({
    ...
    publishedAt: new Date(item.pubDate ?? now),
    fetchedAt: now,
  }));
}
